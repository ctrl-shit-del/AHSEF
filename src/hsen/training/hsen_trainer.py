...
class HSENTrainer:
    ...
    def train_one_epoch(self, loader, epoch: int) -> dict:
        # The KL regulariser's weight ramps over the first epochs, and the loss
        # is otherwise called per batch with no idea which epoch it is in.
        # No-op for the focal head.
    ...
    def evaluate(self, split: str, prefix: str | None = None) -> dict:
        ...
        metrics = evaluate_predictions(
            ...
            head=getattr(self.loss_config, "classification_loss", "focal"),
        )
    ...
    def _finalise(self, elapsed: float, stop_reason: str) -> dict:
        ...
        # On every run, not only when the test split is opened. Calibration,
        # the uncertainty decomposition and anything that needs per-sample
        # confidence are then post-processing over this file rather than a
        # reason to retrain -- which is what CP1's validation-only runs, which
        # left no artefact at all, made obvious.
        self._save_predictions(self.config.run_dir / "validation_predictions.parquet")
    ...
    def _save_predictions(self, path: Path) -> Path:
        ...
        if self.model_config.task == "single_label":
            ...
            # The logits themselves, because a softmax probability is lossy in
            # exactly the ways later work needs: a temperature cannot be
            # refitted from it and a Dirichlet alpha cannot be recovered from
            # it. Storing K more float columns costs nothing next to the
            # features and removes a whole class of "retrain to re-analyse".
            raw = outputs["logits"].numpy()
            for index, name in enumerate(self.class_names):
                frame[f"logit_{name}"] = raw[:, index]
